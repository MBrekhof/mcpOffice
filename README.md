...
- `excel_map_vba_sheet_access(path, moduleName?, sheetName?, includeUnresolved=true, includeRecords=true, maxRecords=300)` — per procedure, which sheet and range / defined name it reads and writes; `ActiveSheet` and unqualified access are reported as unresolved, never guessed. On a big workbook call with `includeRecords=false` first (summary + per-sheet rollup only), then scope the records with `moduleName` / `sheetName`.
...
- `excel_list_vba_form_controls(path, formName?)` — each UserForm's controls inferred from its code-behind (handler names, `Me.` references, Hungarian prefixes or VBE default names, `As MSForms.X` declarations) with type, confidence, events and referenced properties.
