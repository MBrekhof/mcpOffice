...
public class MermaidCallgraphRendererTests
{
    ...
    // ----- Task 12: escaping + structural balance -----
    ...
    public void Subgraph_open_count_matches_end_count()
    {
        ...
        var subgraphCount = Regex.Matches(output, @"^\s*subgraph\b", RegexOptions.Multiline).Count;
        var endCount = Regex.Matches(output, @"^\s*end\s*$", RegexOptions.Multiline).Count;
