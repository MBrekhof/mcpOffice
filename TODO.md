...
- [ ] `VbaProjectReader.ClassifyKind` (line 169) detects worksheet document modules with `entry.Name.StartsWith("Sheet")`, which is English-Excel-only. Dutch Excel uses `Blad1` / `Blad2` / `Blad3` codenames; Italian uses `Foglio`; German `Tabelle`; French `Feuil`; Spanish `Hoja`. On Dutch workbooks (e.g. `RingOnderzoek.xlsm`, `Balans.xlsm`) these land as `classModule` instead of `documentModule`. Fix: detect document modules via the MS-OVBA flags (or the same-named root sheet relationship) rather than a name prefix. Add a Dutch fixture to lock the behavior in.
